...
class billViewController: UIViewController, UISearchResultsUpdating, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate, UINavigationBarDelegate{
    ...
    override func viewDidLoad() {
        ...
        Alamofire.request("http://sample-env-1.kkc6kfpbre.us-west-2.elasticbeanstalk.com/", method:.get, parameters:["data":"bills_active"]).responseJSON { (responseData) -> Void in
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        performSegue(withIdentifier: "activeSegue", sender: each[indexPath.row])
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        let send = segue.destination as! detailViewControllerActive
        send.mickey = sender as! [String:AnyObject]
    }
    
    
    @IBAction func backToPreviousActive(segue: UIStoryboardSegue){}
