...
class comViewControllerJoint: UIViewController, UISearchResultsUpdating, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate, UINavigationBarDelegate{
            ...
            if((responseData.result.value) != nil) {
                ...
                if let resData = swiftyJsonVar["results"].arrayObject {
                    ...
                    self.arrRes = self.all.filter(){
                        return ($0["chamber"]as? String)!.hasPrefix("joint")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell : UITableViewCell = tableView.dequeueReusableCell(withIdentifier: "comCellJ")!
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        performSegue(withIdentifier: "comJsegue", sender: each[indexPath.row])
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        let send = segue.destination as! comDetailViewControllerJ
        send.mickey = sender as! [String:AnyObject]
    }
    
    
    @IBAction func backComJoint(segue: UIStoryboardSegue){}
