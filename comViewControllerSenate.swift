...
class comViewControllerSenate: UIViewController, UISearchResultsUpdating, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate, UINavigationBarDelegate{
            ...
            if((responseData.result.value) != nil) {
                ...
                if let resData = swiftyJsonVar["results"].arrayObject {
                    ...
                    self.arrRes = self.all.filter(){
                        return ($0["chamber"]as? String)!.hasPrefix("senate")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell : UITableViewCell = tableView.dequeueReusableCell(withIdentifier: "comCellS")!
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        performSegue(withIdentifier: "comSsegue", sender: each[indexPath.row])
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        let send = segue.destination as! comDetailViewControllerS
        send.mickey = sender as! [String:AnyObject]
    }
    
    
    @IBAction func backComSenate(segue: UIStoryboardSegue){}
